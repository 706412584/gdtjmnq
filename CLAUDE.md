...
- 设计基准：16:9 横屏，设计分辨率 1920×1080
- 分辨率模式：模式 A（明确设计分辨率），使用 `UI.Scale.DESIGN_RESOLUTION(1920, 1080)` 配合响应式布局
- 安全区域：顶部、底部及左右各预留至少 `3%` 安全边距，移动端主要触控区域不小于 44×44 逻辑像素
- UI 组件库：`urhox-libs/UI`（Yoga Flexbox 布局）
- 发布方向：`.project/project.json` 必须使用 `screen_orientation = "landscape"`
...
| 按钮图 | 256x128 | 可九宫格拉伸 |
| 角色立绘 | 768x1024 | 横屏对话界面半身像，预留左右站位裁切 |
| 场景背景 | 1920x1080 | 16:9 横屏场景背景 |
| 武器图鉴 | 512x512 | 正方形展示 |
| 发布截图 | 1920x1080 | 必须来自真实横屏游戏画面 |
| 宣传图/横版封面 | 1920x1080 | 以真实游戏截图为视觉依据 |
| 游戏图标 | 512x512 | 正方形，无文字小字 |
