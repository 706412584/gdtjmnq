...
-- 角色精灵表路径映射（sheet名 → PNG路径 + 帧数）
-- 文件名用横杠(Soldier-Idle.png)，sheet key 用下划线(Soldier_Idle)
M.sheetMap = {
    Soldier_Idle = { path = "image/topdown-basic/chars/Soldier-Idle.png", count = 6 },
    Soldier_Walk = { path = "image/topdown-basic/chars/Soldier-Walk.png", count = 8 },
    Orc_Idle     = { path = "image/topdown-basic/chars/Orc-Idle.png",     count = 6 },
    Orc_Walk     = { path = "image/topdown-basic/chars/Orc-Walk.png",     count = 8 },
}
...
function M.UpdatePlayer(dt)
    ...
    -- flipX 由 PlayActorAnim 内部通过 textureRect UV 交换处理
...
-- 装饰帧动画（SpriteAnimation 节点）：按 fps 循环切帧，使用 textureRect。
    ...
    for _, a in ipairs(M.config.animSprites) do
        ...
        if spr and a.count > 1 then
            ...
            if st.t >= 1 / a.fps then
                st.t = st.t - 1 / a.fps
                st.f = (st.f + 1) % a.count
                -- 使用 textureRect 替代 SpriteSheet2D
                local sheetInfo = M.sheetMap[a.sheet]
                if sheetInfo and scene2d.setCharFrame then
                    scene2d.setCharFrame(spr, sheetInfo.path, st.f, sheetInfo.count, false)
                end
            end
...
-- 角色/敌人状态机帧动画：使用 textureRect UV 裁剪（绕过 SpriteSheet2D:new() 不可用问题）
...
function M.PlayActorAnim(id, anims, stateName, dt)
    ...
    if not st or st.name ~= stateName then
        st = { name = stateName, t = 0, f = 0, currentSheet = "" }
    end
    ...
    -- 从 sheetMap 获取 PNG 路径
    local sheetInfo = M.sheetMap[clip.sheet]
    if not sheetInfo then return end

    -- 判断朝向
    local flipX = false
    -- 玩家朝向
    if id == M.config.player.id then
        flipX = not M.state.facingRight
    else
        -- 敌人朝向
        flipX = (M.state.enemyDir[id] or 1) < 0
    end

    -- 使用 scene2d 导出的 setCharFrame
    if scene2d.setCharFrame then
        scene2d.setCharFrame(spr, sheetInfo.path, st.f, sheetInfo.count, flipX)
    end
